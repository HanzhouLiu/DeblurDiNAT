...
class Embeddings_output(nn.Module):
    ...
    def forward(self, x, residual_1, residual_2):
        ...
        hx = self.up_sample(torch.cat((ll, lH), dim=1))
...
class FSformer_V3(nn.Module):
    def __init__(self):
        super(FSformer_V3, self).__init__()
...
model = FSformer_V3().cuda().to(dtype=torch.float16)
out = model(inp)
print(out.shape)
