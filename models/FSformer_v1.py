...
class Embeddings_output(nn.Module):
    def __init__(self):
        ...
        self.reduce_chan_level2 = nn.Conv2d(dim*2, dim, kernel_size=1, bias=False)
        self.dwt = DWT_2D(wave='haar')
        self.idwt = IDWT_2D(wave='haar')
        self.enrich_chan_level2 = nn.Conv2d(dim, dim*2, kernel_size=1, bias=False)
...
class FSformer_v1(nn.Module):
    def __init__(self):
        super(FSformer_v1, self).__init__()
        ...
        self.enrich_chan_level1 = nn.Conv2d(dim, dim*2, kernel_size=1, bias=False)
...
model = FSformer_v1().cuda().to(dtype=torch.float16)
out = model(inp)
print(out.shape)
