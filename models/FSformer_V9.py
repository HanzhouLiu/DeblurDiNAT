...
class LoBlock(nn.Module):
    ...
    def attn(self, x):
        ...
        l_q = rearrange(l_q, 'b (head c) h w -> b head (h w) c', head=self.heads)
        l_k = rearrange(l_k, 'b (head c) h w -> b head (h w) c', head=self.heads)
        l_v = rearrange(l_v, 'b (head c) h w -> b head (h w) c', head=self.heads)
...
class FSformer_V9(nn.Module):
    def __init__(self):
        super(FSformer_V9, self).__init__()
