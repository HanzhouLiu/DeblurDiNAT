...
class ChanBlock(nn.Module):
    ...
    def attn(self, x):
        ...
        q_fft = torch.fft.fft(q.float(), dim=-2)
        k_fft = torch.fft.fft(k.float(), dim=-2)
        v_fft = torch.fft.fft(v.float(), dim=-2)
...
class Freqformer_V4(nn.Module):
    def __init__(self):
        super(Freqformer_V4, self).__init__()
