...
if __name__ == "__main__":
    ...
    print("number of GFLOPs: %.2f G"%(model_restoration.flops() / 1e9))  # 12.00 GMacs
"""
"""
import time
start_time = time.time()
inp = torch.randn(1, 3, 256, 256).cuda()#.to(dtype=torch.float16)
model = Uformer().cuda()#.to(dtype=torch.float16)
out = model(inp)
print(out.shape)
...
with torch.cuda.device(0):
  ...
  macs, params = get_model_complexity_info(net, (3, 256, 256), as_strings=True,
                                           print_per_layer_stat=True, verbose=True)
  print('{:<30}  {:<8}'.format('Computational complexity: ', macs))  # 85.72 GMac
  print('{:<30}  {:<8}'.format('Number of parameters: ', params))  # 50.88 M
"""