...
# channel controlled short skip connection
class ShortSkip(nn.Module):
    def __init__(self, bias):
        super(ShortSkip, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv = nn.Conv1d(1, 1, kernel_size=3, padding=1, bias=bias) 
        self.sigmoid = nn.Sigmoid()
        
    def forward(self, x, y):
        z = x + y 
        z_ = self.avg_pool(z)
        z_ = self.conv(z_.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        z_ = self.sigmoid(z_)
        x = x * z_.expand_as(z)
        x = x + y
...
class NADeblur_V3(nn.Module):
    def __init__(self,
                 ...
                 bias = False):
        super(NADeblur_V3, self).__init__()
...
model = NADeblur_V3().cuda()#.to(dtype=torch.float16)
out = model(inp)
print(out.shape)
print("--- %s seconds ---" % (time.time() - start_time))
pytorch_total_params = sum(p.numel() for p in model.parameters())
