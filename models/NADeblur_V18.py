...
class TransBlock(nn.Module):
    ...
    def forward(self, x):
        if self.sa == True:
            ...
            #x = x + attn*self.chan_mod(attn)
            norm = self.norm1(x)
            attn_src = norm*self.chan_mod(norm)
            x = x + self.attn(attn_src)
...
class NADeblur_V18(nn.Module):
    def __init__(self,
                 ...
                 bias = False):
        super(NADeblur_V18, self).__init__()
...
model = NADeblur_V12().cuda()#.to(dtype=torch.float16)
out = model(inp)
print(out.shape)
print("--- %s seconds ---" % (time.time() - start_time))
pytorch_total_params = sum(p.numel() for p in model.parameters())
