...
class Embeddings_output(nn.Module):
    def __init__(self):
        ...
        self.de_block_2 = HiBlock(dim, head_num, 4, 1, False)
        self.de_block_3 = LoBlock(dim, head_num, 4, 1, False)
        self.de_block_4 = HiBlock(dim, head_num, 4, 1, False)
        self.de_block_5 = LoBlock(dim, head_num, 4, 1, False)
        self.de_block_6 = HiBlock(dim, head_num, 4, 1, False)
    ...
    def forward(self, x, residual_1, residual_2):
        ...
        b, c, h, w = hx.shape
        subbands_top = torch.cat((ll, lh), dim=-1) # LL LH
        subbands_bot = torch.cat((hl, hh), dim=-1) # HL HH
        subbands = torch.cat((subbands_top, subbands_bot), dim=-2)
        ll = self.de_block_1(ll)
        subbands = self.de_block_2(subbands)
        ll = self.de_block_3(ll)
        subbands = self.de_block_4(subbands)
        ll = self.de_block_5(ll)
        subbands = self.de_block_6(subbands)
        ll_, lh_, hl_, hh_ = self.dwt(subbands).chunk(4, dim=1)
...
class FSformer_V8(nn.Module):
    def __init__(self):
        super(FSformer_V8, self).__init__()
        ...
        self.Trans_block_2 = HiBlock(dim, head_num, 4, 1, False)
        self.Trans_block_3 = LoBlock(dim, head_num, 4, 1, False)
        self.Trans_block_4 = HiBlock(dim, head_num, 4, 1, False)
        self.Trans_block_5 = LoBlock(dim, head_num, 4, 1, False)
        self.Trans_block_6 = HiBlock(dim, head_num, 4, 1, False)
        self.Trans_block_7 = LoBlock(dim, head_num, 4, 1, False)
        self.Trans_block_8 = HiBlock(dim, head_num, 4, 1, False)
        self.Trans_block_9 = LoBlock(dim, head_num, 4, 1, False)
        self.Trans_block_10 = HiBlock(dim, head_num, 4, 1, False)
        self.Trans_block_11 = LoBlock(dim, head_num, 4, 1, False)
        self.Trans_block_12 = HiBlock(dim, head_num, 4, 1, False)
    ...
    def forward(self, x):
        ...
        subbands = self.Trans_block_2(subbands)
        ll = self.Trans_block_3(ll)
        subbands = self.Trans_block_4(subbands)
        ll = self.Trans_block_5(ll)
        subbands = self.Trans_block_6(subbands)
        ll = self.Trans_block_7(ll)
        subbands = self.Trans_block_8(subbands)
        ll = self.Trans_block_9(ll)
        subbands = self.Trans_block_10(subbands)
        ll = self.Trans_block_11(ll)
        subbands = self.Trans_block_12(subbands)
        ll_, lh_, hl_, hh_ = subbands[:,:,:h//2,:w//2], subbands[:,:,:h//2,w//2:], subbands[:,:,h//2:,:w//2], subbands[:,:,h//2:,w//2:]
        hx = self.idwt(torch.cat((ll, lh_, hl_, hh_), dim=1))
...
model = FSformer_V8().cuda().to(dtype=torch.float16)
out = model(inp)
print(out.shape)
