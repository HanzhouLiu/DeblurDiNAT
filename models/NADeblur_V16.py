...
class EGDFN(nn.Module):
    ...
    def forward(self, x):
        x = self.project_in(x)
        x = self.dwconv(x)
        x1, x2 = (x*self.sigmoid(self.pwconv(x))).chunk(2, dim=1)
        x = F.gelu(x1) * x2
        x = self.project_out(x)
        return x
...
class NADeblur_V16(nn.Module):
    def __init__(self,
                 ...
                 bias = False):
        super(NADeblur_V16, self).__init__()
...
model = NADeblur_V16().cuda()#.to(dtype=torch.float16)
out = model(inp)
print(out.shape)
print("--- %s seconds ---" % (time.time() - start_time))
pytorch_total_params = sum(p.numel() for p in model.parameters())
