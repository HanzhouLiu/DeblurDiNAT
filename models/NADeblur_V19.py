...
class DBGDFN(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias):
        ...
        self.projectIn_br1 = nn.Conv2d(hidden_features, hidden_features*1, kernel_size=1, bias=bias)
        
        self.projectIn_br2 = nn.Conv2d(hidden_features, hidden_features*1, kernel_size=1, bias=bias)
    ...
    def forward(self, x):
        x = self.project_in(x)
        x1, x2 = self.dwconv(x).chunk(2, dim=1)
        x1 = self.projectIn_br1(x1)
        x2 = self.projectIn_br2(x2)
        x1 = self.dwconv_br1(x1)
        x2 = self.dwconv_br2(x2)
        x1 = F.gelu(x1) * x2
        x2 = F.gelu(x2) * x1
...
class NADeblur_V19(nn.Module):
    def __init__(self,
                 ...
                 bias = False):
        super(NADeblur_V19, self).__init__()
