...
class Embeddings_output(nn.Module):
    def __init__(self):
        ...
        self.reduce_chan_level2 = nn.Conv2d(dim*4, dim, kernel_size=1, bias=False)
        self.dwt = DWT_2D(wave='haar')
        self.idwt = IDWT_2D(wave='haar')
        self.enrich_chan_level2 = nn.Conv2d(dim, dim*4, kernel_size=1, bias=False)

        self.de_block_1 = LoBlock(dim, head_num, 2, 1, False)
        self.de_block_2 = HiBlock(dim*3, head_num, 2, 1, False)
        self.de_block_3 = LoBlock(dim, head_num, 4, 1, False)
        self.de_block_4 = HiBlock(dim*3, head_num, 4, 1, False)
        self.de_block_5 = LoBlock(dim, head_num, 8, 1, False)
        self.de_block_6 = HiBlock(dim*3, head_num, 8, 1, False)
...
class FSformer_v0(nn.Module):
    def __init__(self):
        super(FSformer_v0, self).__init__()

        self.encoder = Embeddings()
        head_num = 5
        dim = 320 // 4
        #dim = 320
        self.reduce_chan_level1 = nn.Conv2d(dim*4, dim, kernel_size=1, bias=False)
        self.dwt = DWT_2D(wave='haar')
        self.idwt = IDWT_2D(wave='haar')
        self.Trans_block_1 = LoBlock(dim, head_num, 2, 1, False)
        self.Trans_block_2 = HiBlock(dim*3, head_num, 2, 1, False)
        self.Trans_block_3 = LoBlock(dim, head_num, 2, 1, False)
        self.Trans_block_4 = HiBlock(dim*3, head_num, 2, 1, False)
        self.Trans_block_5 = LoBlock(dim, head_num, 4, 1, False)
        self.Trans_block_6 = HiBlock(dim*3, head_num, 4, 1, False)
        self.Trans_block_7 = LoBlock(dim, head_num, 4, 1, False)
        self.Trans_block_8 = HiBlock(dim*3, head_num, 4, 1, False)
        self.Trans_block_9 = LoBlock(dim, head_num, 8, 1, False)
        self.Trans_block_10 = HiBlock(dim*3, head_num, 8, 1, False)
        self.Trans_block_11 = LoBlock(dim, head_num, 8, 1, False)
        self.Trans_block_12 = HiBlock(dim*3, head_num, 8, 1, False)
        self.enrich_chan_level1 = nn.Conv2d(dim, dim*4, kernel_size=1, bias=False)
...
model = FSformer().cuda().to(dtype=torch.float16)
out = model(inp)
print(out.shape)
