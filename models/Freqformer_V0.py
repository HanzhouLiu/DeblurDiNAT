...
class SpatialBlock(nn.Module):
    def __init__(self, dim, num_heads, win_size, ffn_expansion_factor, bias):
        ...
        self.complex_norm = ComplexNorm(type='spatial')
    ...
    def attn(self, x):
        ...
        attn = (q_fft.transpose(-2, -1) @ k_fft) * self.temperature  # b head (h w) (patch1 patch2//2+1) (patch1 patch2//2+1)
        
        attn = self.complex_norm(attn.view(attn.shape[:-1]+(self.patch_size, self.patch_size//2+1)))  # b head (h w) (patch1 patch2//2+1) patch1 patch2//2+1
        attn = attn.view(attn.shape[:-2]+(-1,))  # b head (h w) (patch1 patch2//2+1) (patch1 patch2//2+1)
        
        out = attn @ (v_fft.transpose(-2, -1))  # b head (h w) (path1 patch2//2+1) c
        out = out.transpose(-2, -1)  # b head (h w) c (patch1 patch2//2+1) 
...
class Freqformer_V0(nn.Module):
    def __init__(self):
        super(Freqformer_V0, self).__init__()
