...
class LoBlock(nn.Module):
    ...
    def forward_attn(self, x):
        myList = []
        for t in range(4):
            myList.append(self.attn(x))
        myList = torch.stack(myList)
        x = torch.mean(myList, dim=0)
        return x
...
class SpikeDeblur(nn.Module):
    def __init__(self):
        super(SpikeDeblur, self).__init__()
