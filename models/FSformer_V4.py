...
class Embeddings_output(nn.Module):
    def __init__(self):
        ...
        self.up_sample = Upsample(dim, dim)
    ...
    def forward(self, x, residual_1, residual_2):
        ...
        hx = self.fusion(torch.cat((ll, hx), dim=1))
...
class FSformer_V4(nn.Module):
    def __init__(self):
        super(FSformer_V4, self).__init__()
...
model = FSformer_V4().cuda().to(dtype=torch.float16)
out = model(inp)
print(out.shape)
