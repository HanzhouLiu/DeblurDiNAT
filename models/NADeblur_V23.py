...
class TransBlock(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=1, bias=False, LayerNorm_type='WithBias'):
...
class Embeddings_output(nn.Module):
    def __init__(self, dim, num_blocks, num_refinement_blocks, heads, bias):
        ...
        self.de_trans_level3 = nn.Sequential(*[
            item for sublist in 
            [[TransBlock(dim*2**2, 1, bias=bias),
             TransBlock(dim*2**2, 1, bias=bias)] for i in range(num_blocks[2]//2)] for item in sublist
        ])
        ...
        self.de_trans_level2 = nn.Sequential(*[
            item for sublist in 
            [[TransBlock(dim*2, 1, bias=bias),
             TransBlock(dim*2, 1, bias=bias)] for i in range(num_blocks[1]//2)] for item in sublist
        ])
        ...
        self.de_trans_level1 = nn.Sequential(*[
            item for sublist in 
            [[TransBlock(dim, 1, bias=bias),
             TransBlock(dim, 1, bias=bias)] for i in range(num_blocks[0]//2)] for item in sublist
        ])
...
class NADeblur_V23(nn.Module):
    def __init__(self,
                 ...
                 bias = False):
        super(NADeblur_V23, self).__init__()
...
model = NADeblur_V23().cuda()#.to(dtype=torch.float16)
out = model(inp)
print(out.shape)
print("--- %s seconds ---" % (time.time() - start_time))
pytorch_total_params = sum(p.numel() for p in model.parameters())
