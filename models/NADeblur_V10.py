    ...
    def __init__(self, dim, num_blocks, num_refinement_blocks, heads, bias):
        ...
        self.de_trans_level3 = nn.Sequential(*[
            ...
            [[TransBlock(dim*2**2, heads[2], 7, 1, 1, bias=bias),
             TransBlock(dim*2**2, heads[2], 7, 1, 1, bias=bias)] for i in range(num_blocks[2]//2)] for item in sublist
        ])
        ...
        self.de_trans_level2 = nn.Sequential(*[
            ...
            [[TransBlock(dim*2, heads[1], 7, 1, 1, bias=bias),
             TransBlock(dim*2, heads[1], 7, 1, 1, bias=bias)] for i in range(num_blocks[1]//2)] for item in sublist
        ])
        ...
        self.de_trans_level1 = nn.Sequential(*[
            ...
            [[TransBlock(dim, heads[0], 7, 1, 1, bias=bias),
             TransBlock(dim, heads[0], 7, 1, 1, bias=bias)] for i in range(num_blocks[0]//2)] for item in sublist
        ])
...
class NADeblur_V10(nn.Module):
    def __init__(self,
                 ...
                 bias = False):
        super(NADeblur_V10, self).__init__()
...
model = NADeblur_V10().cuda()#.to(dtype=torch.float16)
out = model(inp)
print(out.shape)
print("--- %s seconds ---" % (time.time() - start_time))
pytorch_total_params = sum(p.numel() for p in model.parameters())
