...
class Trainer:
    ...
    def train(self):
        ...
        if os.path.exists(os.path.join(model_dir, 'last_FSformer_gopro.pth')):
            print('load_pretrained')
            training_state = (torch.load(os.path.join(model_dir, 'last_FSformer_gopro.pth')))
            ...
            training_state = (torch.load(os.path.join(model_dir, 'final_FSformer_pretrained.pth')))
            new_weight = self.netG.state_dict()
